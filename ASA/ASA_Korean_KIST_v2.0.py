...
try:
    ...
    block_dir = os.path.join(current_tank, block_name)  # 탱크/블록 경로
...
STIM_DIR = os.path.join(SCRIPT_DIR, 'Stimuli_48k_cos')

data_filename = os.path.join(
    block_dir,  # 탱크 블록 폴더에 저장
...
text_center = visual.TextStim(
    win, text='', font=FONT, color='white', height=0.1,
    wrapWidth=WRAP_CENTER, alignText='center',
...
text_question = visual.TextStim(
    win, text='', font=FONT, color='white', height=0.1,
    wrapWidth=WRAP_LEFT, alignText='left',
    anchorHoriz='center', anchorVert='center',
)
...
for _, cb_row in this_rows.iterrows():
    ...
    for half in range(1, NUM_HALVES + 1):
        ...
        for trial_pos in range(start_idx, end_idx):
            ...
            # Attention rating (211=onset, 221=응답)
            ...
            # Comprehension question (212=onset, 222=응답)
